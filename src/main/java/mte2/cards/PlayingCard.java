...
public class PlayingCard  implements Comparable<PlayingCard> {
    ...
    public int compareTo(PlayingCard other){
        ...
        if (this.suit.equals(other.suit)){
            if( this.rank.getRankNumber()-other.rank.getRankNumber() == 0){
                diff = 0;
            }
            else if(this.rank.getRankNumber()-other.rank.getRankNumber() < 0){
                diff = -1;
            }
            else{diff = 1;}
        }
        else if((this.suit.toString().compareTo(other.suit.toString()))<0){
            diff = -1;
        }
        else{diff = 1;}
        return diff;
